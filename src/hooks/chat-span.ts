...
import { addUsageAttributes } from '../gen-ai-span-attributes.js';
...
export function createChatSpan(
  ...
  if (inputMessagesForChatSpan.length > 0) {
    attributes['gen_ai.input.messages'] = prepareForCapture(inputMessagesForChatSpan);
  }
  if (llmEntry.systemInstructions.length > 0) {
    attributes['gen_ai.system_instructions'] = prepareForCapture(llmEntry.systemInstructions);
  }
  ...
  if (outputMessages.length > 0) {
    attributes['gen_ai.output.messages'] = prepareForCapture(outputMessages);
  }
